...
def _generate_html_content(data: Dict[str, Any], json_filename: str) -> str:
    ...
    # Format timestamps from JSON data (convert to local timezone)
    start_time_str = data.get('start_time', '')
    end_time_str = data.get('end_time', '')
    
    # Parse and format timestamps
    def format_timestamp(ts) -> str:
        """Convert timestamp to local timezone and format."""
        if not ts:
            return 'N/A'
        ...
        try:
            # Try ISO format first (with or without 'T', with or without timezone)
            # Replace 'Z' with '+00:00' for UTC
            ts_clean = ts.replace('Z', '+00:00').strip()
            
            # Try parsing as ISO format
            if 'T' in ts_clean or '+' in ts_clean or ts_clean.count('-') >= 3:
                dt = datetime.fromisoformat(ts_clean)
            else:
                # Try parsing as "YYYY-MM-DD HH:MM:SS" format (from str() conversion)
                dt = datetime.strptime(ts_clean, '%Y-%m-%d %H:%M:%S.%f')
            
            # If timezone-aware, convert to local time; otherwise assume already local
    ...
    start_time = format_timestamp(start_time_str)
    end_time = format_timestamp(end_time_str)
    
    # Use end_time for report generation time, or start_time if end_time is not available
    report_generated_time = end_time if end_time != 'N/A' else start_time
...
<head>
    ...
    <title>Vhape Test Report - {report_generated_time}</title>
    <style>
        {_get_css()}
    </style>
</head>
            ...
            <div class="header-content">
                ...
                <div class="header-meta">
                    <div class="meta-item">
                        ...
                        <span class="meta-value">{report_generated_time}</span>
                    </div>
                    <div class="meta-item">
...
def _generate_security_issues_section(security_issues: list) -> str:
    ...
    def format_timestamp(ts) -> str:
        ...
        if isinstance(ts, str):
            # Try to parse and format ISO timestamp
            try:
    ...
    for issue in security_issues:
        ...
        <div class="security-issue security-{severity}">
            ...
            {f'<div class="issue-timestamp">{formatted_timestamp}</div>' if formatted_timestamp else ''}
        </div>
        '''
